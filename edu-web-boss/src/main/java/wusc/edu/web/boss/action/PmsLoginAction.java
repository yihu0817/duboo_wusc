...
import wusc.edu.web.mqservice.ProducerService;
import wusc.edu.web.redisservice.RedisService;
import wusc.edu.web.redisservice.utils.SerializeUtil;
	...
	public String userLogin() {
		try {
			...
			if (user.getUserPwd().equals(DigestUtils.sha1Hex(pwd))) {
				...
				//发送登录成功消息到消息队列
				log.info(Thread.currentThread().getName()+"------------send to jms Start");
				String msg = user.getUserName()+"登录入平台,时间："+user.getCreateTime();
		        producer.sendMessage(msg);
		        log.info(Thread.currentThread().getName()+"------------send to jms End");
		      //从缓存中获取用户信息显示
		        log.info("保存登录用户信息到redis缓存");
				redisService.set(user.getUserName().getBytes(), SerializeUtil.serialize(user));
				return "main";
