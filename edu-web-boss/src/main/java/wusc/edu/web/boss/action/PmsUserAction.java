...
public class PmsUserAction extends BaseAction {
	...
	public String userViewOwnInfo() {
		try {
			...
			PmsUser user = null;
			byte[] bytes = redisService.gets(pmsUser.getUserName());
			if(bytes != null){
				user = (PmsUser) SerializeUtil.unserialize(bytes);
				log.info("从redis缓存中读数据");
			}
			if(user == null)
				user = pmsUserFacade.getById(pmsUser.getId());
			...
			//从缓存中获取用户信息显示
			//redisService.set(pmsUser.getUserName().getBytes(), SerializeUtil.serialize(user));
